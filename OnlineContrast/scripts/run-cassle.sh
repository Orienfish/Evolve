...
if [ $2 = "stream51" ] ; then
  ...
  if [ $3 = "seq" ]; then
    python main_supcon.py --size $size --lifelong_method cassle --criterion $1 --dataset $2 --model $model --training_data_type class_instance \
  ...
  if [ $3 = "seq-im" ]; then
    python main_supcon.py --size $size --lifelong_method cassle --criterion $1 --dataset $2 --model $model --training_data_type class_instance --imbalanced \
      --batch_size 128 --mem_samples 128 --mem_size $mem_size \
      --val_batch_size 64 --num_workers 8 --steps_per_batch_stream 10 --print_freq 10 --epochs 1 \
      --learning_rate_stream $lr --temp_cont 0.1 \
      --train_samples_ratio 0.4 --test_samples_ratio 0.9 --val_samples_ratio 0.1 --knn_samples_ratio 1.0 \
